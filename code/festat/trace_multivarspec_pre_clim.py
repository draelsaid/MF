...
exp = ['6BRZ_PRECISE_AD','68NQ_20130716']
nbfc= ['120','96']
listlabel = ['PRECISE','AROME']
nbxplev =[106,90,]
param = ['q','t', 'v', 'd' ]
...
couleur = ["k--","b-","g--","r"]
listlev=[86,65]
listlenght=[5940,1927]
chainetitre='varspec_pre_872_moy'
...
for p in range(0,len(param)):
	...
	for i in range(0,len(exp)):
		...
		while nres <= NMAXRES:
			...
			#lecture
			nom_fic = ("/home/broussea/ASSIM/jbens/JB_" + exp[i] + "_" + nbfc[i] + "/varsp" + param[p] + str(lev) + ".y" )
			with open(nom_fic, 'r') as f:
				tab = f.readlines()
